...
// import React, { useEffect, useState } from "react";
// import { BookOpen, Plus, Trash2, Layers, BookMarked, Users, GraduationCap, X, LayoutList, Activity, CheckCircle2, Image as ImageIcon, Edit3 } from "lucide-react";
// import { supabase } from "../../../supabase";
// import toast from "react-hot-toast";
...
//       // 2. Safely fetch student enrollments
//       const { data: enrollmentsData } = await supabase
//         .from("student_enrollments")
//         .select(`
//           student_id,
...
//   //     // Instantly update local modal view and background list
//   //     await fetchCourseDetails(selectedCourse.id);
//   //     fetchCourses();
//   //   } catch (err) {
//   //     toast.error("Failed to add phase: " + err.message);
...
//         .from("course_phases")
//         .insert([{ 
//           course_id: selectedCourse.id, // <-- Properly linking to this course
//           title: newPhaseTitle.trim(), 
//           phase_number: nextPhaseNumber 
//         }])
//         .select()
...
//       // Refreshes the deep dive modal data and main course card list
//       await fetchCourseDetails(selectedCourse.id);
//       fetchCourses();
//     } catch (err) {
//       toast.error("Failed to add phase: " + err.message);
...
//               <div
//                 key={course.id}
//                 className="rounded-2xl border border-slate-100 dark:border-white/10 bg-slate-50/50 dark:bg-white/5 hover:bg-white dark:hover:bg-white/10 hover:shadow-md transition-all flex flex-col justify-between overflow-hidden group"
//               >
//                 <div>
//                   {/* Thumbnail Banner */}
//                   <div className="h-36 w-full bg-slate-200 dark:bg-slate-800 relative overflow-hidden flex items-center justify-center">
...
//                             <div className="p-4 space-y-2">
//                               {phase.course_lessons?.sort((a,b) => a.position - b.position).map((lesson, idx) => (
//                                 <div key={lesson.id} className="flex items-center gap-3 p-3 rounded-xl hover:bg-slate-50 dark:hover:bg-white/5 border border-transparent hover:border-slate-100 dark:hover:border-white/10 transition-colors">
//                                   <div className="text-slate-300 dark:text-slate-600 font-black text-sm w-6">{(idx + 1).toString().padStart(2, '0')}</div>
//                                   <div className="flex-1 text-sm font-bold text-slate-700 dark:text-slate-300">{lesson.title}</div>
//                                 </div>
//                               ))}
...
//     await handleCreatePhase(e);
//   }} 
//   className="p-5 bg-blue-50 dark:bg-blue-500/5 border border-blue-100 dark:border-blue-500/20 rounded-2xl flex flex-col sm:flex-row items-stretch sm:items-center gap-4"
// >
//   <div className="flex-1">
//     <label className="block text-[10px] font-black uppercase tracking-wider text-blue-600 dark:text-blue-400 mb-1">Add New Phase</label>
//     <input
...
// }
// // import React, { useEffect, useState } from "react";
// // import { BookOpen, Plus, Trash2, Layers, BookMarked, Users, GraduationCap, X, LayoutList, Activity, CheckCircle2, Image as ImageIcon } from "lucide-react";
// // import { supabase } from "../../../supabase";
// // import toast from "react-hot-toast";
...
// //     try {
// //       setLoading(true);
// //       // Clean query focusing purely on courses and their structural phases/lessons
// //       const { data, error } = await supabase
// //         .from("courses")
// //         .select(`
// //           *,
...
// //       // Fetch user counts separately to prevent relationship join constraints from failing
// //       const { data: usersData } = await supabase.from("users").select("id, role, assigned_course_id");
...
// //       const { error } = await supabase
// //         .from("course_phases")
// //         .insert([{ course_id: selectedCourse.id, title: newPhaseTitle, phase_number: (selectedCourse.course_phases?.length || 0) + 1 }]);
...
// //       const { error } = await supabase
// //         .from("course_lessons")
// //         .insert([{ phase_id: phaseId, title: title, position }]);
...
// //           </h2>
// //           <p className="text-xs text-slate-400 font-bold mt-1">
// //             Manage course tracks, thumbnail previews, phases, structured lessons, and monitor enrollments.
// //           </p>
// //         </div>
// //         <button
// //           onClick={() => setIsCreating(!isCreating)}
...
// //                       {/* Add Phase Form */}
// //                       <form onSubmit={handleCreatePhase} className="p-5 bg-blue-50 dark:bg-blue-500/5 border border-blue-100 dark:border-blue-500/20 rounded-2xl flex items-center gap-4">
// //                         <div className="flex-1">
// //                           <label className="block text-[10px] font-black uppercase tracking-wider text-blue-600 dark:text-blue-400 mb-1">Add New Phase</label>
// //                           <input
// //                             type="text"
...
// //                             value={newPhaseTitle}
// //                             onChange={(e) => setNewPhaseTitle(e.target.value)}
// //                             className="w-full p-3 text-sm rounded-xl border border-white dark:border-white/10 shadow-sm font-bold focus:outline-blue-500 dark:bg-slate-900 dark:text-white"
// //                           />
// //                         </div>
// //                         <button type="submit" disabled={!newPhaseTitle.trim()} className="mt-5 px-5 py-3 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white rounded-xl font-black text-xs uppercase tracking-wider transition-all">
// //                           Create Phase
// //                         </button>
// //                       </form>
// //                     </div>
...
// //                       ) : (
// //                         <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
// //                           {selectedCourse.enrolled_users?.map((student) => {
// //                             const progress = calculateStudentProgress(student);
// //                             return (
// //                               <div key={student.id} className="p-5 bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-2xl flex flex-col gap-4 shadow-sm hover:shadow-md transition-all">
// //                                 <div className="flex items-start justify-between">
