...
import { User, Lock, Bell, Shield, Save, Loader2, Sun, Moon, LogOut, Camera } from "lucide-react";
import toast from "react-hot-toast";

export default function StudentSettingsView({ user }) {
  ...
  // Sync theme state with user profile on load
  useEffect(() => {
    if (user?.id) {
      supabase
        .from("users")
  ...
  // Toggle Theme Function & Save to Database
  const toggleTheme = async () => {
    ...
    // Save preference to Supabase profile row
    if (user?.id) {
      await supabase
        .from("users")
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto space-y-8">
      ...
      <div>
        ...
        <p className="text-sm text-slate-500 mt-1">Manage your appearance, student profile preferences, security, and session options.</p>
      </div>
      ...
      <div className="bg-white dark:bg-white/5 border border-slate-100 dark:border-white/10 rounded-3xl p-8 shadow-sm flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="font-bold text-slate-900 dark:text-white text-base">Interface Theme</h2>
      ...
      {/* Profile Information & Avatar Section */}
      <div className="bg-white dark:bg-white/5 border border-slate-100 dark:border-white/10 rounded-3xl p-8 shadow-sm">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h2 className="font-bold text-slate-900 dark:text-white text-base">Profile & Avatar</h2>
      ...
      <div className="bg-white dark:bg-white/5 border border-slate-100 dark:border-white/10 rounded-3xl p-8 shadow-sm">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h2 className="font-bold text-slate-900 dark:text-white text-base">Security & Password</h2>
      ...
      <div className="bg-white dark:bg-white/5 border border-slate-100 dark:border-white/10 rounded-3xl p-8 shadow-sm">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400">Manage how you receive alerts and coursework updates.</p>
          </div>
        </div>
        ...
        <div className="space-y-4">
          <label className="flex items-center justify-between p-4 rounded-2xl bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/5 cursor-pointer">
            <div>
              ...
              <p className="text-xs text-slate-400">Receive announcements regarding classes and homework updates.</p>
            </div>
            <input 
              type="checkbox" 
          ...
          <label className="flex items-center justify-between p-4 rounded-2xl bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/5 cursor-pointer">
            <div>
              <p className="font-bold text-slate-800 dark:text-white text-sm">Exam Reminders</p>
              <p className="text-xs text-slate-400">Get notified when new tests or upcoming assessments are published.</p>
            </div>
            <input 
              type="checkbox" 
      ...
      <div className="bg-red-50/50 dark:bg-red-500/5 border border-red-100 dark:border-red-500/20 rounded-3xl p-8 shadow-sm flex items-center justify-between">
        <div>
          <h2 className="font-bold text-red-600 dark:text-red-400 text-base">Terminate Session</h2>
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Sign out securely from your student portal account on this device.</p>
        </div>
        <button 
